...
x_t = graph(align='left',width=333,height=300,     #畫x-t圖                                  
              title='K-Purple, U-Green, E-Balck', xtitle='t', ytitle='x',
              foreground=color.black,background=color.white,
              xmax=8, xmin=0)
f1_1 = gcurve(color=color.green)       
f1_2 = gcurve(color=color.purple)
f1_3 = gcurve(color=color.black)
v_t = graph(align='left',width=333,height=300,   #畫v-t圖                                  
              title='P1-red, P2-blue, P-black', xtitle='t', ytitle='vx',
              foreground=color.black,background=color.white,
...
while True :
    rate(1000)
    if mag(ball1.pos - ball2.pos) < spring_L:
    ...
    ball1.v += vector(ball1_a, 0, 0) * dt
    ball2.v += vector(ball2_a, 0, 0) * dt
    ...
    if mag(ball1.pos - ball2.pos) < spring_L:
        ...
        u = 0
    k = 0.5 * (m1 * ball1.v.x * ball1.v.x + m2 * ball2.v.x * ball2.v.x)
    p1 = m1 * ball1.v.x
    p2 = m2 * ball2.v.x
