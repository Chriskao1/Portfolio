...
while True:
    ...
    if mag(ball_a) > 0:
        a_vector.axis = ball_a/mag(ball_a)*R/2
    v_text.pos = v_vector.pos + v_vector.axis*1.2 #文字標籤'v'的位置
    a_text.pos = a_vector.pos + a_vector.axis*1.2
    print("simulated acceleration =", mag(ball_a), ", theoretical acceleration =", mag(ball.v) * mag(ball.v) / R)
